...
def summarize_splits(splits, weighted=True):
    ...
    v.sort(); v.reverse()
    #opt = scipy.log(v[0][0])
    opt = (decimal.Decimal(v[0][0])).ln()

    rows.append(["split", "lnL", "Rel.Prob"])
    sumprob = 0.0
    for L, split in v:
        L_decimal = decimal.Decimal(L)
        lnL = L_decimal.ln()
        #lnL = scipy.log(L)
        relprob = (L/ptot)
        #if sumprob < 0.95:
...
def ancsplits_fullML(outfile, tree, model, params, paramsDict, nodelabels=None, verbose=True, tee=False):
...
"""
        log(s, outfile, tee)
    for node in [ x for x in tree.root.iternodes(phylo.PREORDER) if (not x.istip) ]:
        label = node.label
        print label
        if (nodelabels and (label in nodelabels)) or (not nodelabels):
            ...
            v = []
            for dist in [ d for d in model.dists[1:] if d not in node.excluded_dists ]:
                x = optimize.ancdist_likelihood_fullML( node, dist, model, params, paramsDict)
                v.extend(x)
...
def ancsplits(outfile, tree, model, dispersal, extinction, nodelabels=None, verbose=True, tee=False):
    # nodelabels is assumed to be a list of strings
    if verbose:
        s = """
Ancestral range subdivision/inheritance scenarios ('splits') at
...
def ancsplits_mp(outfile, tree, model, params, nodelabels=None, verbose=True, tee=False):
    # nodelabels is assumed to be a list of strings
    if verbose:
        s = """
Ancestral range subdivision/inheritance scenarios ('splits') at
...
# This is the new function that we will need in this file.
def optimize_dispersal_extinction_De(outfile, tree, model, tee=False):
    params, nlnL = optimize.optimize_De(tree,model)
    lutLen = len(model.lut)
    D = model.array2Dmatrix(params[0:lutLen-1])
    e = params[lutLen]
    log("Global ML at root node:\n  -lnL = %.4g" % nlnL, outfile, tee)
    #print >> outfile, "  dispersal = %.4g\n  extinction = %.4g" % (d, e)
    log("  extinction = %.4g" % (e), outfile, tee)
    return D, e
