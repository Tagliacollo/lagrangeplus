...
##                 node.parent.age = node.age + node.length
    ...
    def eval_likelihood(self):
        ...
        if self.root.sumFail == True:
            ancdist_conditional_lh_dec(self.root)
            return float(-(sum(self.root.dist_conditionals_dec).ln()) )
        else:
            return -math.log(scipy.sum(self.root.dist_conditionals))
...
def conditionals_dec(node):
    ...
    # tips, this is a zeroed vector with 1.0 at the observed range
    distconds     = node.segments[0].dist_conditionals
    distconds_dec = node.segments[0].dist_conditionals_dec
    ...
    for seg in node.segments:
        ...
        P, P_dec      = model.P_dec(seg.period, seg.duration)
        v             = scipy.zeros((model.ndists,))
        v_dec         = decimalarray.DecimalArray(v)
        for i in seg.distrange:
            # P[i] is the vector of probabilities of going from dist i
            # to all other dists
            v[i]     = sum(distconds * P[i])
            v_dec[i] = sum(distconds_dec * P_dec[i])

        distconds     = v
        distconds_dec = v_dec

    return distconds, distconds_dec

def ancdist_conditional_lh_dec(node):
    """
    recursive calculation of fractional likelihoods for dists at
    internal nodes in a tree
    """
    if node.sumFail == False:
        node.segments[0].dist_conditionals_dec = decimalarray.DecimalArray(node.segments[0].dist_conditionals)
        return
    ...
    if not node.istip:
        ...
        ancdist_conditional_lh_dec(c1)
        ancdist_conditional_lh_dec(c2)

        v1, v1_dec = conditionals_dec(c1)
        v2, v2_dec = conditionals_dec(c2)

        distconds     = scipy.zeros((model.ndists,))
        distconds_dec = decimalarray.DecimalArray(distconds)
        ancsplits     = []
        for distidx, dist in model.enumerate_dists():
            lh     = 0.0
            lh_dec = decimal.Decimal(0.0)
            if dist not in node.excluded_dists:
                for ancsplit in model.iter_ancsplits(dist):
                    d1, d2       = ancsplit.descdists
                    lh_part      = (v1[dist2i[d1]]     * v2[dist2i[d2]]    )
                    lh_part_dec  = (v1_dec[dist2i[d1]] * v2_dec[dist2i[d2]])
                    lh          += (lh_part     *         ancsplit.weight )
                    lh_dec      += (lh_part_dec * decimal.Decimal(ancsplit.weight))
                    ancsplit.likelihood = lh_part
                    ancsplits.append(ancsplit)

            distconds[distidx]     = lh
            distconds_dec[distidx] = lh_dec
        node.ancsplits = ancsplits
    ...
    if node.parent:
        node.segments[0].dist_conditionals     = distconds
        node.segments[0].dist_conditionals_dec = distconds_dec
        #print 'lagrange.tree.ascdist_conditional_lh'
        #print 'node.label = ',node.label
        #for i in range(len(node.segments[0].dist_conditionals)):
            #print node.segments[0].dist_conditionals[i], node.segments[0].dist_conditionals_dec[i]
        #print '########################'
        #print
    else:
        node.dist_conditionals      = distconds
        node.dist_conditionals_dec  = distconds_dec
        #print 'lagrange.tree.ascdist_conditional_lh'
        #print 'node.label = ',node.label
        #for i in range(len(node.dist_conditionals)):
            #print node.dist_conditionals[i], node.dist_conditionals_dec[i]
        #print '########################'
        #print

def print_ascdist(node, model):

    if node.istip:
        return 0

    c1, c2 = node.children
    print_ascdist(c1, model)
    print_ascdist(c2, model)

    ranges = model.dists

    print 'Node.label = ',node.label
    print 'Node.sumFail = ',node.sumFail

    if node.sumFail and node.parent:
        nodeDistConds    = node.segments[0].dist_conditionals_dec

    if node.sumFail and not node.parent:
        nodeDistConds    = node.dist_conditionals_dec

    if not node.sumFail and node.parent:
        nodeDistConds    = decimalarray.DecimalArray(node.segments[0].dist_conditionals)

    if not node.sumFail and not node.parent:
        nodeDistConds    = decimalarray.DecimalArray(node.dist_conditionals)

    negLogConds = [-x.ln() for x in nodeDistConds]
    minLogConds = min(negLogConds)

    print 'min negLogConds = ',minLogConds
    
    for i in range(len(negLogConds)):
        if negLogConds[i] < minLogConds + 2:
            print ranges[i],': ',negLogConds[i]
