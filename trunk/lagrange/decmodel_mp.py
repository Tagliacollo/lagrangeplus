...
class DECModel:
    ...
    def P_dec(self, period, t):
        k = (period, t)
        P_dec = list()
        for row in self.Pdict[k]:
            P_dec.append(decimalarray.DecimalArray(row))
        self.Pdict_dec[k] = P_dec
        return self.Pdict[k], P_dec
...
class DECModel_De(DECModel):

    def setup_D(self, Dmatrix=None):
        if Dmatrix is None:
            Dmatrix = self.Dmask
        self.D = Dmatrix
    ...
    def mkArray2DmatrixLUT(self, dm):
        self.lut = list()
        ldm = scipy.array(dm)
        for t in range(0,self.nTime):
            for r in range(0,self.nRow):
                for c in range(0,self.nCol):
                    if ldm[(t,r,c)]:
                        self.lut.append( (t,r,c) )

    def Dmatrix2Array(self, dm):
        ret = []
        for i in range(0,len(ret)):
            ret.append( dm[ self.lut[i] ])
        return ret

    def array2Dmatrix(self, arr):
        ret = scipy.zeros((self.nTime, self.nRow, self.nCol))
        for i in range(0,len(arr)-1):
            ret[ self.lut[i] ] = arr[i]
        return(ret)
