import sys, os, nexus, scipy, numpy
from decmodel_mp import DECModel, DECModel_De
...
def eval_decmodel_De(s):
    ...
    dm = d["area_dispersal"]
    periods = d["dispersal_durations"]
    model = DECModel_De(nareas, labels, periods=periods, dists=dists, Dmask=scipy.array(dm))
    #model.Dmask[:] = dm
    model.nTime = len(dm)
    model.nRow  = len(dm[0])
    model.nCol  = len(dm[0][0])
    model.mkArray2DmatrixLUT(dm)
    trees = d["newick_trees"]
    newicktree = trees[0]["newick"]
    nodelabels = trees[0]["included"]
    root_age = trees[0]["root_age"] or None
